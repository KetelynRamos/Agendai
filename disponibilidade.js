document.querySelectorAll('.day-setting input[type="checkbox"]').forEach(checkbox => {
    checkbox.addEventListener('change', (event) => {
        const parent = event.target.closest('.day-setting') || event.target.closest('.day-card');
    ...
    saveBtn.addEventListener('click', () => {
        ...
        dayCards.forEach(card => {
            ...
            if (checkbox && checkbox.checked) {
                const day = checkbox.id || checkbox.value.toLowerCase();
                const startInput = card.querySelector(`[id$="-start"]`) || card.querySelectorAll('.time-input')[0];
                const endInput = card.querySelector(`[id$="-end"]`) || card.querySelectorAll('.time-input')[1];
